		...
		try {
			...
			if(Utils.getOptionPos('x',options) >= 0) {
				...
				if (lname != null) {
					...
					//ADDED UTILS FOR PRINTING TO TSV
					String fname = "testResults.tsv";

					File checkFile = new File(fname);
					StringBuilder rString = new StringBuilder();
					HashMap <String,Object> y = Result.getStats(r, voption);
					
					if (checkFile.exists()){
						String input = readFile(fname);
						rString.append(input);
						rString.append(MLUtils.hashMapToTSV2(y, 5));
						...
						rString.append(MLUtils.hashMapToTSV(y, 3));
						String x = rString.toString();
					...
					SerializationHelper.write("text.txt", r);
				}
