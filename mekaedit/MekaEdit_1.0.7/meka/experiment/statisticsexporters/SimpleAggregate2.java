  ...
  implements AggregatedEvaluationStatisticsExporter {
	...
	protected EvaluationStatistics doAggregate(List<EvaluationStatistics> stats) {
		...
		for (String key: keys) {
			...
			TDoubleArrayList tempValue = new TDoubleArrayList();
			TDoubleArrayList stdevValue = new TDoubleArrayList();
			for (EvaluationStatistics stat: stats) {
				...
				if (stat.get("Run").doubleValue()== tempRun)
					tempValue.add(stat.get(key).doubleValue());
				else{
					stdevValue.add(Utils.mean(tempValue.toArray()));
					tempValue = new TDoubleArrayList();
					tempValue.add(stat.get(key).doubleValue());
					tempRun+=1;
				}
			}
			...
			if (values.size() > 0) {
				...
				if (!m_SkipStdDev)
					result.put(key + m_SuffixStdDev, Math.sqrt(Utils.variance(stdevValue.toArray())));
			}
		}
