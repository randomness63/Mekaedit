 ...
 * @version 2012.02.27
 */
public class DropoutNeuralNet implements ExtendedNeuralNet, Serializable {
    ...
    private ArrayList<ArrayList<Double>> dropoutMatrix;
     ...
     */
    public DropoutNeuralNet(int[] netTopology, double biasInput,
    ...
    //defaults to dropout false
    public double[] feedForward(final double[] inputPattern){
    	return feedForward(inputPattern, false);
    }
   
    public double[] feedForward(final double[] inputPattern, boolean dropout) {
        ...
        for (int layerIndex = 0; layerIndex < layers.size(); layerIndex++) {
            ...
            ArrayList<Double> dLayer = dropoutMatrix.get(layerIndex);
            
            int layerSize = layer.size();
            layerOutput = new double[layerSize];
            
            //multiplies feedforward by corresponding dropout coeff
            if (layerIndex!= layers.size()-1) {

				// ensures position on dropoutMatrix is the same as that of neuron topology
				for (int n = 0; n < layerSize; n++) {
					
					double dropCoeff = 0;
					
					// checks chance of dropout
					if (dropout){
						dropCoeff = dLayer.get(n);
					}else{
						dropCoeff = 1;
					}
					
					//creates new layer with dropout applied
					if (layerIndex == 0) {
						layerOutput[n] = layer.get(n).processInput(new double[] { layerInput[n] }) * dropCoeff;
					} else {
						layerOutput[n] = layer.get(n).processInput(layerInput) * dropCoeff;
					}
				} 
			}else{
				//dropout is not applied for input layer and output layer
				for (int n = 0; n < layerSize; n++) {
					if (layerIndex == 0) {
    ...
    //sets the output matrix for this instance
    public void setDropoutMatrix(ArrayList<ArrayList<Double>> dMatrix) {
    	this.dropoutMatrix = dMatrix;
    }
