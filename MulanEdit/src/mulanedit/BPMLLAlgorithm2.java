 ...
 * @see NeuralNet
 */
public class BPMLLAlgorithm2 {

    private final ExtendedNeuralNet neuralNet;
    private final double weightsDecayCost;
    private final int[] netTopo;

    /**
     * Creates a {@link BPMLLAlgorithm} instance. Does not work with dropout.
     *
     ...
     */
    public BPMLLAlgorithm2(ExtendedNeuralNet neuralNet, double weightsDecayCost) {
        if (neuralNet == null) {
    ...
    /**
     * Creates a {@link BPMLLAlgorithm} instance. Intended for use with dropout.
     * Sets the netTopology of this instance for dropout
     *
     * @param neuralNet the neural network model to learn
     * @param weightsDecayCost the weights decay cost term used for regularization.
     * 					The value must be greater than 0 and no more than 1.
     * @param netToplogy is the topology of the neural net architecture
     * 					This is an array of integers representing the number
     * 					of neurons in each layer
     */
    public BPMLLAlgorithm2(ExtendedNeuralNet neuralNet, double weightsDecayCost, int[] netTopology) {   	
        if (neuralNet == null) {
            throw new IllegalArgumentException("The passed neural network model is null.");
        }
        if (weightsDecayCost <= 0 || weightsDecayCost > 1) {
     ...
     */
    public ExtendedNeuralNet getNetwork() {
        return neuralNet;
    }
    ...
    public double learn(double[] inputPattern, double[] expectedLabels, double learningRate){
    	return learn(inputPattern, expectedLabels,learningRate, 1);
    }
     ...
     * 		  instance, then i-th value is +1, otherwise the value is -1.
     * @param learningRate the learning rate used to update the neural network weights
     * @param dropChance the percentage of neurons removed in the curent pass.
     * @return the error of the network response for the passed input
     * 			or {@link Double#NaN} if the passed input can not be processed.
     */
    public double learn(double[] inputPattern, double[] expectedLabels, double learningRate, double dropChance) {
    	ArrayList<ArrayList<Double>> dMatrix = null;
        ...
        //0. CREATE DROPOUT @author jason
        if (netTopo!= null){
        	Dropout dMat = new Dropout(netTopo, dropChance);
        	dMatrix = dMat.getDropoutMatrix();
        }
        
		// 1. PROPAGATE SIGNAL
        //resets dropout matrix for each iteration -j
        neuralNet.setDropoutMatrix(dMatrix);
        double[] networkOutputs = neuralNet.feedForward(inputPattern, true);
        ...
        // 2. UPDATE WEIGHTS - error back-propagation
        ...
        for (int layerIndex = layersCount - 1; layerIndex > 0; layerIndex--) {
            ...
            //specific layer of dmatrix here into function
            ArrayList<Double> dLayer = dMatrix.get(layerIndex);
            ArrayList<Double> inputLayer = dMatrix.get(layerIndex - 1);
            updateWeights(layer, previousLayerOut, learningRate, inputLayer, dLayer);
        }
    ...
    private void updateWeights(List<Neuron> layer, double[] layerInputs, double learningRate, ArrayList<Double> inputDropout, ArrayList<Double> layerDropout) {

        // w(t+1) = w(t) + a*dw(t) + m*w(t-1) ... dw(t) = e(t)*in(t)
        int layerSize = layer.size();
        //for each neuron
        for (int n = 0; n < layerSize; n++) {
        	double dropoutWeight = layerDropout.get(n);
        	
        	
        	//if current neuron is active,
        	if (dropoutWeight != 0){
        		...
        		//this updates all input layers
        		for (int i = 0; i < inputsCount; i++) {
        			//if input is active
        			if (inputDropout.get(i) != 0){
